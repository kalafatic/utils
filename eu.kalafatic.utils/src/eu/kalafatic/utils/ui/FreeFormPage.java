...
public class FreeFormPage extends FormPage {
	...
	private void createExpandable(final ScrolledForm form,
		...
		final ExpandableComposite exp = toolkit.createExpandableComposite(
		...
		final Button button = toolkit.createButton(client,
				Messages.getString("FreeFormPage.button"), SWT.PUSH); //$NON-NLS-1$
	...
	private void createFormTextSection(final ScrolledForm form,
		...
		section.setDescription(Messages
				.getString("FreeFormPage.sectionDescription")); //$NON-NLS-1$
		TableWrapData td = new TableWrapData();
		td.align = TableWrapData.FILL;
	...
	private void loadFormText(final FormText rtext, FormToolkit toolkit) {
		rtext.addHyperlinkListener(new HyperlinkAdapter() {
			@Override
			public void linkActivated(HyperlinkEvent e) {
				MessageDialog.openInformation(rtext.getShell(),
						Messages.getString("FreeFormPage.mtitle"), //$NON-NLS-1$
